            ...
            <div class="card">
                <div class="card-header">{{ __('Личный кабинет') }}</div>
                    <div class="card-body">
                        ...
                        <a href="{{ route('advertiser.workboard' ) }} "  class="link-primary link-offset-2 link-underline-opacity-25 link-underline-opacity-100-hover"> <p style="text-align:center"> Вернуться назад<p></a>  
                    </div>     {{--card-body --}}
                </div>      {{--card --}}
        </div>
            ...
            $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $.each(data.offers, function (key, value) {
                           ...
                           $("#table tbody").append("<tr><td>"+value.title+"</td><td>" + value.cpc+"</td><td>"+value.clicks+"</td><td>"+$is_actived+"</td><td>"+(value.clicks*value.cpc).toFixed(2)+"</td></tr>");
                           sum = sum + value.clicks*value.cpc;            // считаем общую сумму расходов
                        });
                        sum = (sum).toFixed(2);
