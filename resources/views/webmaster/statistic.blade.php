    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                ...
                <a href="{{  route('webmaster.workboard') }}"  class="link-primary link-offset-2 link-underline-opacity-25 link-underline-opacity-100-hover"> <p style="text-align:center"> Вернуться назад<p></a>  
            </div>
        </div>
</div>  {{--class="container" --}}
...
<script>
    ...
    function ajax(period) {
        $.ajax({
            url: '{{ route('webmaster.ajax' ) }}',              // localhost/webmaster/workboard/sum
            type: 'POST',
            dataType: 'json',
            data: {
                period: period,
            ...
            success: function(data) {
                ...
                $.each(data.offers, function (key, value) {
                    ...
                    $("#table tbody").append("<tr><td>"+value.title+"</td><td>" + value.cpc+"</td><td>"+value.clicks+"</td><td>"+is_actived+"</td><td>"+(value.clicks*value.cpc*0.8).toFixed(2)+"</td></tr>");
                    sum = sum + value.clicks*value.cpc*0.8;            // считаем общую сумму доходов
                });
                sum = (sum).toFixed(2); 
